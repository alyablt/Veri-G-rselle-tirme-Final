...
# Veri çerçevesini uzun formata dönüştürme
data_long <- reshape2::melt(nomofobi, id.vars = "Bolum", 
                            measure.vars = c("Akilli_Telefon_Kullanım_Yili", "Gunluk_Akilli_Telefon_Sure"),
                            variable.name = "Degisken", 
                            value.name = "Deger")


# ggplot2 ile kutu grafikleri oluşturma (Bölüme Göre Akıllı Telefon Kullanım Yılı ve Süresinin Dağılımı)
ggplot(data_long, aes(x = Bolum, y = Deger, fill = Bolum)) +
  geom_violin(outliers = FALSE, staplewidth = 0.5, alpha=0.6) +
  facet_wrap(~ Degisken, scales = "free_y") +
  stat_summary(fun.y=mean, geom="point", shape=30, size=2, color="red", fill="red") +
  labs(title = "Bölüme Göre Akıllı Telefon Kullanım Yılı ve Süresinin Dağılımı",
       x = "Bölüm",
       y = "Değer") +
  theme_minimal() +
  theme(legend.position = "none", axis.text.x = element_text(angle = 10, vjust = 1, 
...
ggplot(nomofobi, aes(x = NOMOFOBİ_TOPLAM, y = Akilli_Telefon_Kullanım_Yili, fill = Bolum)) +
  geom_boxplot(outliers = FALSE, staplewidth = 0.5, alpha=0.9) +
  labs(x = "Nomofobi", y = "Akıllı Telefon Kullanım Yılı", title = "Bölüme ve Akıllı Telefon Yılına Göre Nomofobi Düzeyi") +
  theme_minimal()+
